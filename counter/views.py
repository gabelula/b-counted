...
#def CheckContributor(user):
#  check = UserStat.objects.filter('contributor = ', user)
...
def Index(request):
  return render_to_response('why.html')

###### display the form to add an event

@login_required
def CountMe(request):
  form = InputForm()
  form.created_by = request.user
  return render_to_response('form.html',{'form':form, 'ratioform': RatioForm(),'user': request.user})
...
def CountNow(request):  
  ...
  if request.method =='GET':
    if request.GET.get('ShowMultisession','') == '1' and not request.GET.get('Event',''):
      get_stat = UserStat.all().filter('Contributor =',request.user)
      if get_stat.count() > 0:
